...
- **Playwright** (`e2e/`): the production build in Chromium, Firefox, WebKit and an iPhone 15 profile. First time: `npx playwright install chromium firefox webkit`.
  ...
  - Firefox runs with autoplay allowed (`firefoxUserPrefs` in `playwright.config.js`): its autoplay policy doesn't count Playwright's clicks as a gesture and leaves `AudioContext.resume()` pending, so Web Audio bells waited on the frozen clock's 1s fallback forever.
