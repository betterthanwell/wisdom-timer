...
- **Bell patterns**: `startStrikes`, `intervalStrikes`, `endStrikes` (1-3, default 1). Strikes are 5 s apart for start/end bowls, 2 s for the interval woodblock (`BELL_STRIKE_SPACING_MS`). Reset cancels strikes not yet rung; pause doesn't. **The whole "Bell strikes" section is only shown while interval bells are on** (owner's choice), and its switch (`showBellStrikes`, default off, saved) shows/hides the 1×/2×/3× choices. Hiding is UI only: saved strike counts - including start/end - keep applying.
