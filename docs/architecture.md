...
- `pause()` takes the time left from the clock, not the (possibly stale) displayed value, to the millisecond (`remainingMsRef`): durations may have fractions of a second (guided meditations), so resuming must not round. `timeRemaining` is shown in whole seconds, rounded up. `onStart(elapsed)` gets the time sat so far.
...
- `init()` is idempotent (React StrictMode mounts twice in dev) and delegates to `loadSounds()`: bells load first (waiting at most 2s in total, `BELL_LOAD_WAIT_MS`), then the ambient element is created. "Loading bells…" shows and Start stays disabled until it's done.
...
- Ambient: one looping element. The first `unlock()` routes it through Web Audio (`routeAmbientThroughWebAudio()`: media element source → `ambientGain` → speakers, element volume left at 1), so the slider, fades and gentle ending work on iOS; `ambientOutput()`/`setAmbientOutput()` use the gain once routed, the element's volume before (or without Web Audio). Once routed, its sound needs the audio context running (`playAmbient()` asks it to resume). `fade()` runs a fixed 20 steps over 500ms and always finishes, even where the browser ignores `volume` (iOS); the fade-in reads the target volume every step. A new sound started during a fade-out cancels it (`playAmbient()` clears the fade): a cancelled fade-out leaves the element alone - otherwise its last step would pause the new sound - and its `stopAmbient()` promise still resolves (`fade()`'s `onEnd(finished)`).
- The same element plays a guided meditation's recording (ambient sounds don't play in guided mode): `playAmbient(id, from)` with a guided id plays it once (`loop` off) from `from` seconds, also when resuming it. It's primed in the Start tap like an ambient sound after settling in, since the voice starts from the lead-in's timer.
- Interruptions (guided voice): `setInterruptionListener(fn)` is told the voice's position when something outside the app stops it - the element's `pause` event while the app didn't pause it (`ambientPausedByApp`) and the recording hasn't ended - or when the context goes `interrupted` (the voice is paused there; `null` before the voice started). Reported once per stop (`interruptionReported`), cleared when it plays again. `cutShortVoicePosition()` answers the same question on demand, for a page that wakes up past the end. `useTimer.pauseAt(elapsed)` pauses at that point, also taking back a completion from inside `onComplete`.
- After any reported interruption (`needsFreshAudio`), the next `unlock()` - the Carry on / Start tap - calls `rebuildAudio()`: closes the old context and builds a new one with a new bell gain (decoded bells are kept; `AudioBuffer`s work in any context) and a new ambient element (an element stays tied to the context it was routed through), then routes it as usual. Whatever was playing stops; the app starts it again in the same tap. On an iPhone, after a call, the voice stayed silent on the old pipeline even though Carry on resumed the session.
